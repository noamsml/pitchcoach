...
public class RangePitchDetect extends Activity implements IPitchReciever {
	...
	private void dialogAndFinish(double pitch) {
		...
		if (intLow >= intHigh){
			builder.setMessage("The highest pitch you sang was lower than the lowest! Please try again!")
			.setCancelable(false)
			.setNegativeButton("Try Again", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int id) {
					finish();
					Intent myIntent = new Intent(getApplicationContext(), RangePitchDetect.class);
