...
public class ImageSource {
	...
	public ImageSource(int width, int height, double target) {
		...
		canvas.drawLine((float) 0, targetPos, (float) width, targetPos,
				targetPaint);
	}
	...
	private RectF makeRectF(double left, double top, double right, double bottom) {
		return new RectF((float) left, (float) top, (float) right,
				(float) bottom);
	}

	public synchronized void addDatapoint(double pitch, double time) // Assume
																		// time
																		// is
																		// never
																		// greater
																		...
																		// SYN
	{
		...
		if (this.current_x + pixelwidth > this.width) {
			drawBrokenDatapoint(this.current_x, this.current_x + pixelwidth,
					this.current_y, pixely, linePaint);
			this.current_x = this.current_x + pixelwidth - this.width;
		} else {
			drawDatapoint(this.current_x, this.current_x + pixelwidth,
					this.current_y, pixely, linePaint);
	...
	private void drawDatapoint(double curx, double nextx, double cury,
			double nexty, Paint linePaint) {
		canvas.drawRect(this.makeRectF(curx, 0, nextx, height), bgPaint);
		canvas.drawLine((float) curx, targetPos, (float) nextx, targetPos,
				targetPaint);
		canvas.drawLine((float) curx, (float) (height - cury), (float) nextx,
				(float) (height - nexty), linePaint);
	}

	private void drawBrokenDatapoint(double curx, double nextx, double cury,
			double nexty, Paint linePaint) {
	...
	public synchronized void straightWrite(Canvas c, int otherwidth,
			int otherheight) {
		// TODO: Implement for realz
		if (!onceAround)
			c.drawBitmap(bitmap, null, makeRect(0, 0, width, height), null);
		else {
			c.drawBitmap(bitmap, makeRect(current_x, 0, width, height),
					makeRectF(0, 0, width - current_x, height), null);
			c.drawBitmap(bitmap, makeRect(0, 0, current_x, height),
					makeRectF(width - current_x, 0, width, height), null);
		}
	}

	/*
	 * Updates the line color depending on margin of error. GREEN good, RED bad,
	 * graduate to YELLOW in between. For even MORE visual feedback, target line
	 * turns BLUE when good
	 */
