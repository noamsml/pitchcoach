...
public class TutorialActivity extends PitchActivityFramework {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myEventStream = new PitchSequence(new String[] { "C#3", "E3", "C4",
				"D3", "C3", "E3", "E#3", "D4", "C#3" });
		...
		return new PromiseFactoryPromise(new IPromiseFactory() {
			public Promise getPromise() {
				...
				for (int i = 0; i < steps.length; i++) {
					notes[i] = new Note(playmanager.currentGraph()
							.getFrequency()
							* Math.pow(LetterNotes.stepVal, steps[i]), 0.4);
				}
				notes[notes.length - 1] = new Note(playmanager.currentGraph()
						.getFrequency(), 0.8);
	...
	public Promise tutorial() {
		...
		return playmanager
				.begin()
				.then(new DialogPromise(this,
						"Welcome to pitchcoach! This program should teach you how to sing."))
				.then(new DialogPromise(
						this,
						"Brace yourself! I'm about to play a note. When I'm done, I want you to try and sing it for me."))
				.then(playVerifyLoop())
				.then(new DialogPromise(
						this,
						"Good job! Notice how the line onscreen matches the note you need to sing. Let's try it a few more times."))
				.then(Promise.nTimes(3, new IPromiseFactory() {
					public Promise getPromise() {
						return playmanager.addGraph(false).then(
								playVerifyLoop());
					}
				}))
				.then(new DialogPromise(this, "OK! Let's try an interval!"))
				.then(new DialogPromise(
						this,
						"For intervals or scales, I'll play all the notes you need to sing, and then play the first note you need to sing again afterwards, to start you off."))
				.then(new DialogPromise(this,
						"This interval is a C3 to E3 interval. Give it a try."))
				.then(playmanager.addGraph(true))
				.then(new Runnable() {
					public void run() {
				...
				.then(playmanager.addGraph(false).then(playmanager.play()))
				.then(new DialogPromise(this, "Great job!"))
				.then(new DialogPromise(
						this,
						"The Life Bar [tm] (Awesome name, isn't it? We thought of it all by ourselves) shows you how much life you have left. If it fills up, you win! If it empties out, you lose! Let's try playing with the life bar on."))
				.then(new Runnable() {
					public void run() {
						enableLifeBar();
					}
				...
				.then(new DialogPromise(
						this,
						"Cool! You've just finished the entire tutorial. Congrats! Click 'OK' to go back to the home screen"))
				.then(new Runnable() {
					public void run() {
						finish();
					}
