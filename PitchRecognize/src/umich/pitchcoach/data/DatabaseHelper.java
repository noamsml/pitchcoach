...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public long countLessonTypes() {
		Cursor dataCount = myDatabase
				.rawQuery("select * from LessonType", null);
		return dataCount.getCount();
	}

	public long countPerformances() {
		Cursor dataCount = myDatabase.rawQuery("select * from Performance",
				null);
		return dataCount.getCount();
	}

	public Cursor getPitchById(long id) {
		Cursor pitch = myDatabase.rawQuery("select * from Pitch where _id = "
				+ id, null);
		return pitch;
	}

	public long getPitchIdFromFreq(double freq) {
		Cursor pitches = myDatabase.rawQuery(
				"select * from Pitch where frequency = " + freq, null);
		pitches.moveToFirst();
	...
	public Cursor getAllPitchesOrderedRandom() {
		Cursor pitches = myDatabase.rawQuery(
				"select * from Pitch order by RANDOM()", null);
		return pitches;
	}

	public Cursor getAllPitchesOrderedLeastRecent() {
		Cursor pitches = myDatabase.rawQuery(
				"select * from Pitch order by 'latestPerformanceDate' ASC",
				null);
		return pitches;
	}
	...
	public Cursor getAllPitchesOrderedLowestRating() {
		Cursor pitches = myDatabase.rawQuery(
				"select * from Pitch order by 'averageRating' ASC", null);
		return pitches;
	}

	public Cursor getAllPitchesInRange(double min, double max) {
		Cursor pitches = myDatabase.rawQuery(
				"select * from Pitch where frequency >= " + min
						+ " AND frequency <= " + max + " order by RANDOM()",
				null);
		return pitches;
	}
	...
	public Cursor getRecentPitchesInRange(double min, double max) {
		Cursor pitches = myDatabase.rawQuery(
				"select * from Pitch where frequency >= " + min
						+ " AND frequency <= " + max
						+ " order by latestPerformanceDate asc", null);
		return pitches;
	}

	public void updateLessonType(long id, long perfId, double averageRating,
			long perfCount, String date) {
	...
	public void updatePitch(long id, long perfId, double averageRating,
			long perfCount, String date) {
		ContentValues pv = new ContentValues();
		pv.put("latestPerformance", perfId);
		pv.put("averageRating", averageRating);
	...
	public Cursor getLessonTypeById(long id) {
		Cursor lessonTypes = myDatabase.rawQuery(
				"select * from LessonType where _id = " + id, null);
		return lessonTypes;
	}

	public Cursor getAllLessonTypesOrderedRandom() {
		Cursor lessonTypes = myDatabase.rawQuery(
				"select * from LessonType order by RANDOM()", null);
		return lessonTypes;
	}

	public Cursor getAllLessonTypesOrderedLeastRecent() {
		Cursor lessonTypes = myDatabase
				.rawQuery(
						"select * from LessonType order by 'latestPerformanceDate' asc",
						null);
		return lessonTypes;
	}
	...
	public Cursor getAllLessonTypesOrderedLowestRating() {
		Cursor lessonTypes = myDatabase.rawQuery(
				"select * from LessonType order by 'averageRating' ASC", null);
		return lessonTypes;
	}

	public Cursor getLessonTypeByName(String name) {
		Cursor lessonType = myDatabase.rawQuery(
				"select * from LessonType where 'name' = " + name, null);
		return lessonType;
	}

	public Cursor getLessonTypeEasy() { // Notes
		Cursor lessonType = myDatabase
				.rawQuery(
						"select * from LessonType where name like '%Note%' order by RANDOM()",
						null);
		return lessonType;
	}

	public Cursor getLessonTypeMedium() { // Notes and Intervals
		Cursor lessonType = myDatabase
				.rawQuery(
						"select * from LessonType where name not like '%Chord%' and name not like '%Scale%' order by RANDOM()",
						null);
		return lessonType;
	}
	...
	public Cursor getLessonTypeChord() {
		Cursor lessonType = myDatabase.rawQuery(
				"select * from LessonType where name like '%Chord%'", null);
		return lessonType;
	}

	public Cursor getLessonTypeScale() {
		Cursor lessonType = myDatabase.rawQuery(
				"select * from LessonType where name like '%Scale%'", null);
		return lessonType;
	}
	...
	public long addPerformance(long lessonId, long pitchId, double rating) {
		...
		performancevalues.put("performanceDate",
