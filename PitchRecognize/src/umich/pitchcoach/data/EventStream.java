...
public class EventStream implements IPitchSource {
	...
	public Event getNextEvent() {
		...
		if (lessonType.moveToFirst()) {
			nextEvent.maxInterval = lessonType.getInt(lessonType
					.getColumnIndex("maxInterval"));
			setVocalRangeFromFile();
			Cursor pitch = myDbHelper.getAllPitchesInRange(minfreq,
					maxFreqWithInterval(nextEvent.maxInterval));
	...
	private void setEvent(Event nextEvent, Cursor lessonType, Cursor pitch) {
		nextEvent.lessonId = lessonType.getLong(lessonType
				.getColumnIndex("_id"));
		nextEvent.pitchId = pitch.getLong(pitch.getColumnIndex("_id"));
		nextEvent.name = pitch.getString(pitch.getColumnIndex("name")) + " "
				+ lessonType.getString(lessonType.getColumnIndex("name"));
		nextEvent.duration = lessonType.getInt(lessonType
				.getColumnIndex("duration"));
		int pitchCount = lessonType.getInt(lessonType
				.getColumnIndex("pitchCount"));
		ArrayList<String> pitchesToSing = new ArrayList<String>(pitchCount);
		ArrayList<Float> intervals = deserializeIntervalList(lessonType
				.getString(lessonType.getColumnIndex("intervals")));
		Note[] notesToPlay;
		if (pitchCount > 1) {
		...
		notesToPlay[0] = new Note(basePitch, nextEvent.duration
				/ (double) pitchCount);
		for (int i = 1; i < pitchCount; i++) {
			...
			notesToPlay[i] = new Note(freq, nextEvent.duration
					/ (double) pitchCount);
		}
		if (pitchCount > 1) {
	...
	private void setDifficultyFromFile() {
		...
		try {
			FileInputStream fis = myContext
					.openFileInput(RangeSelect.DIFFICULTY_FILENAME);
			int ch;
			while ((ch = fis.read()) != -1)
				inb.append((char) ch);
			fis.close();
	...
	public static void submitEventPerformance(String pitch, Event event,
			long rating) {
		if (event != null) {
			myDbHelper.openDataBase();
			long perfId = myDbHelper.addPerformance(event.lessonId,
					event.pitchId, rating);
			if (perfId != -1) {
				Performance perf = new Performance();
				perf.performanceId = perfId;
				perf.lessonId = event.lessonId;
				perf.pitchId = event.pitchId;
				perf.performanceDate = new Timestamp(new Date().getTime())
						.toString();
	...
	private static void updateLessonTypePerformance(long lessonId, long rating,
			Performance perf) {
		Cursor lesson = myDbHelper.getLessonTypeById(lessonId);
		if (lesson.moveToFirst()) {
			double averageRating = lesson.getDouble(lesson
					.getColumnIndex("averageRating"));
			int perfCount = lesson.getInt(lesson
					.getColumnIndex("performanceCount"));
			averageRating = ((perfCount * averageRating) + rating)
					/ (double) (perfCount + 1);
			myDbHelper.updateLessonType(lessonId, perf.performanceId,
					averageRating, perfCount, perf.performanceDate);
		}
	}

	private static void updatePitchPerformance(long pitchId, long rating,
			Performance perf) {
		Cursor pitch = myDbHelper.getPitchById(pitchId);
		if (pitch.moveToFirst()) {
			double averageRating = pitch.getDouble(pitch
					.getColumnIndex("averageRating"));
			int perfCount = pitch.getInt(pitch
					.getColumnIndex("performanceCount"));
			averageRating = ((perfCount * averageRating) + rating)
					/ (double) (perfCount + 1);
			myDbHelper.updatePitch(pitchId, perf.performanceId, averageRating,
					perfCount, perf.performanceDate);
		}
	}
	...
	private Cursor selectPitch() {
		if (rangeIsSet) {
			...
			return myDbHelper.getAllPitchesInRange(DEFAULT_MIN_FREQ,
					DEFAULT_MAX_FREQ);
		}
	}
