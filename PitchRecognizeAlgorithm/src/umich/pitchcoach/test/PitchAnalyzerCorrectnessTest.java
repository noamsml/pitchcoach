...
public class PitchAnalyzerCorrectnessTest {
	public static void main(String[] args) {
		...
		float[][] opatterns = new float[][] { new float[] { 5000 },
		...
		for (int freqidx = 0; freqidx < freqs.length; freqidx++) {
			...
			for (int opatternidx = 0; opatternidx < opatterns.length; opatternidx++) {
				...
				for (int noise = 0; noise < 400; noise += 100) {
					NoisyOvertonePattern pat = new NoisyOvertonePattern(freq,
							44100, opattern, noise);
					double res = 0;
					try {
					...
					if (Math.abs(diff) < 6) {
						System.out
								.format("PASSED: freq %f Hz, opattern #%d, noise at %d\n",
										freq, opatternidx, noise);
					} else {
						System.out
								.format("FAILED %.2f: freq %f Hz, opattern #%d, noise at %d\n",
