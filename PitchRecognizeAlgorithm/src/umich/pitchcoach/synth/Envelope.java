...
public class Envelope extends Wave {
	...
	public double getSample(int sampleNum) {
		if (sampleNum < lenEnvelope)
			return orig.getSample(sampleNum) * ((double) sampleNum)
					/ lenEnvelope;
		if (sampleNum > len - lenEnvelope)
			return orig.getSample(sampleNum) * ((double) (len - sampleNum))
					/ ((double) lenEnvelope);
